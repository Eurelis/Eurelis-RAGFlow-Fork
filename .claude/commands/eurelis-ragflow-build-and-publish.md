...
- `/eurelis-ragflow-build-and-publish v0.25.0-eurelis.1` — utilise le tag passé en paramètre (`$ARGUMENTS`)
...
**Si `$ARGUMENTS` est fourni** (ex: `/eurelis-ragflow-build-and-publish v0.25.0-eurelis.1`) :
...
1. Vérifier qu'un tag Eurelis est posé sur HEAD : `git tag --points-at HEAD --list "v*-eurelis.*"`. Si absent, **arrêter** :
   ...
   > # ou : /eurelis-ragflow-build-and-publish vX.Y.Z-eurelis.N
   > ```
