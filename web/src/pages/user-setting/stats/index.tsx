// Eurelis — Page de statistiques de consommation personnelle (user-setting).
...
import type { UserStatsTimeseriesBySourcePoint } from '@/services/user-stats-service';
import {
  Bar,
  BarChart,
  CartesianGrid,
...
import {
  getUserStatsMe,
  getUserStatsMeBreakdown,
  getUserStatsMeSources,
  getUserStatsMeTimeseries,
...
import type { StatsDateRange } from '@/components/time-window-picker';
...
function pivotBySource(rows: UserStatsTimeseriesBySourcePoint[]): {
...
const UserStatsKeys = {
  me: (fromDate: string, toDate: string, source: string, type: string) =>
    ['user/stats/me', fromDate, toDate, source, type] as const,
  timeseries: (
    fromDate: string,
    toDate: string,
    source: string,
    ...
    [
      'user/stats/me/timeseries/by-source',
      fromDate,
      toDate,
      source,
      type,
  ...
  modelBreakdown: (
    ...
    ['user/stats/me/breakdown/model', fromDate, toDate, source, type] as const,
  sources: () => ['user/stats/me/sources'] as const,
};
...
export default function UserStats() {
  ...
  const { data: dimensionsData } = useQuery({
    queryKey: UserStatsKeys.sources(),
    queryFn: async () => (await getUserStatsMeSources()).data.data,
    staleTime: Infinity,
    retry: false,
  });
  ...
  const { data } = useQuery({
    queryKey: UserStatsKeys.me(fromDate, toDate, sourceKey, typeKey),
    queryFn: async () =>
      (
        await getUserStatsMe({
          fromDate,
          toDate,
          source: sourceFilter,
          type: typeFilter,
  ...
  const { data: timeseriesData } = useQuery({
    queryKey: UserStatsKeys.timeseries(fromDate, toDate, sourceKey, typeKey),
    queryFn: async () =>
      (
        await getUserStatsMeTimeseries({
          fromDate,
          toDate,
          source: sourceFilter,
          type: typeFilter,
  ...
  const { data: modelBreakdown } = useQuery({
    queryKey: UserStatsKeys.modelBreakdown(
      fromDate,
      toDate,
      sourceKey,
      typeKey,
  ...
  const { pivoted: series, sources: seriesSources } = useMemo(
    () =>
      pivotBySource(
        (timeseriesData?.series ?? []) as UserStatsTimeseriesBySourcePoint[],
      ),
    [timeseriesData],
  );
    ...
    <ProfileSettingWrapperCard
      header={
        <div className="flex items-center gap-4 flex-wrap">
          <Title>{t('setting.usageStats')}</Title>
          <div className="ml-auto flex items-center gap-2 flex-wrap">
            ...
            <TimeWindowPicker onChange={setRange} />
          </div>
        </div>
      }
    >
                ...
                >
                  ...
                  <XAxis
                    ...
                    tickFormatter={(v: string) =>
                      formatPeriodLabel(v, timeseriesData?.granularity)
                    }
                  />
                  <YAxis
                    tick={TICK_STYLE}
                  ...
                  <Tooltip
                    ...
                    labelFormatter={(label: string) =>
                      formatPeriodLabel(label, timeseriesData?.granularity)
                    }
