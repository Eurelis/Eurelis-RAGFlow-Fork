...
class TenantMgr:
    ...
    def add_member(tenant_id: str, user_id: str, role: str = "normal") -> dict:
        ...
        # Validate the target user exists (avoid dangling memberships).
        user_ok, _ = UserService.get_by_id(user_id)
        if not user_ok:
            raise ValueError(f"User '{user_id}' does not exist.")

        # Validate the team (tenant) exists.
        tenant_ok, _ = TenantService.get_by_id(tenant_id)
        if not tenant_ok:
            raise ValueError(f"Team '{tenant_id}' does not exist.")

        now = current_timestamp()

        # Look at membership rows of ANY status for this (tenant, user) pair.
        # remove_member soft-deletes (status=INVALID) rather than deleting, so a
        # prior membership survives in the table. Reactivate it instead of
        # inserting a second row, otherwise a remove -> re-add cycle accumulates
        # duplicate rows (one INVALID, one VALID) for the same pair.
        with DB.connection_context():
            existing = list(
        ...
        if any(r.status == StatusEnum.VALID.value for r in existing):
            raise ValueError("User is already a member of this team.")

        if existing:
            # Reactivate exactly one soft-deleted row (leave any other stale
            # INVALID rows untouched — they are harmless and filtered out by
            # every VALID-scoped query).
            target = existing[0]
            UserTenantService.filter_update(
                [UserTenant.id == target.id],
                {
                    "role": role,
