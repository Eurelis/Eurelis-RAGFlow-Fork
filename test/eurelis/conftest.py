...
@pytest.fixture(scope="session")
def ref_dataset_id(token: str) -> str:
    """id du dataset de référence (eurelis-ref) — cible du retrieval /datasets/search."""
    def find() -> str | None:
        r = httpx.get(
    ...
    if not dataset_id:
        ...
        seed.main()
        dataset_id = find()
    assert dataset_id, "dataset de référence introuvable même après seed"
    return dataset_id


@pytest.fixture(scope="session")
def ref_search_id(token: str) -> str:
    """id de la Search App de référence (eurelis-ref-search). Auto-seed si absente."""
    def find() -> str | None:
        r = httpx.get(
            f"{HOST_ADDRESS}/api/{VERSION}/searches",
            params={"keywords": REF_SEARCH_NAME},
            headers={"Authorization": f"Bearer {token}"},
            timeout=HTTP_TIMEOUT,
        )
        apps = (r.json().get("data") or {}).get("search_apps") or []
        return next((s["id"] for s in apps if s["name"] == REF_SEARCH_NAME), None)
    ...
    if not sid:
        ...
        seed.main()
        sid = find()
    assert sid, "search de référence introuvable même après seed"
    return sid
