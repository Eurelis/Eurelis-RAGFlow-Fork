...
def stream_search_completion(client: httpx.Client, search_id: str, question: str, **extra) -> list:
    """
    Résumé SSE d'une Search App. Emprunte un chemin différent du chat
    (`/searches/{id}/completions`, corps `{question}`, sans `session_id`).
    Renvoie la liste des frames décodées (dicts ou bool pour la sentinelle).
    """
    payload = {"question": question}
    payload.update(extra)
    frames = []
    with client.stream("POST", f"/api/{VERSION}/searches/{search_id}/completions", json=payload) as r:
        r.raise_for_status()
        for line in r.iter_lines():
            if not line or not line.startswith("data:"):
                continue
