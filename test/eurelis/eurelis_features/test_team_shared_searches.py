...
def team_setup(token):
    ...
    # Le rattachement d'équipe est en cohérence à terme : au tout premier listing après
    # l'acceptation de l'invite, get_joined_tenants_by_user_id (côté search_api) peut ne pas
    # encore refléter l'appartenance de B. On attend que sa vue se stabilise (il voit le search
    # `team`) avant de livrer le contexte — sinon les tests « membre » s'exécuteraient sur une vue
    # vide : faux négatif pour `sees_team_search`, faux POSITIF pour `does_not_see_private_search`.
    for _ in range(30):  # settle borné ~15 s ; au-delà, on livre quand même et le test tranche
        if any(s["id"] == team_search_id for s in _list_searches(b)):
            break
        time.sleep(0.5)
