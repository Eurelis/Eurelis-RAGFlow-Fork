...
├── shield_contract/      # p0 — endpoints consommés par le Shield (SSE, sessions, chats, documents, search)
├── eurelis_fixes/        # p1 — correctifs (chats équipe, retrieval)
└── eurelis_features/     # p1 — évolutions (usage-stats, supervision, PII, partage search équipe)
```
...
- **Contrat Shield (p0)** : flux SSE des complétions (enveloppe, streaming/finale, sentinelle, `reference`,
  ...
  **endpoints Search (feature 028)** : liste `/searches` (métadonnées, `search_config` hors liste),
  détail `/searches/{id}` (flags `kb_ids`/`summary`/`related_search`), retrieval `/datasets/search`
  (chunks aux **noms internes**), résumé SSE `/searches/{id}/completions` (référence **enrichie**),
  related `/chat/recommendation`.
...
- **Évolutions (p1)** : stats de conso user + admin (schéma, cohérence des agrégats, **delta llm/embedding**),
  ...
  **masquage PII** (opt-in `::pii`, via journal d'audit), **merge `llm_factories.patch.json`** (patch synthétique
  monté dans la stack → catalogue fusionné vérifié via l'API), **partage d'équipe des search apps**
  (permission `me|team` — un membre voit les search `team` du tenant, pas les `me` ; contrat de l'objet
  consommé par le Shield).
